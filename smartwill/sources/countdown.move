...
fun init(_otw: COUNTDOWN, ctx: &mut TxContext) {
    let vault = Vault {
        ...
        warned: false,
        timeleft: 6 * 30 * 24 * 60 * 60 * 1000, // 6 months
    };
...
public fun update_time(_cap: &OwnerCap, vault: &mut Vault, clock: &Clock) {
    ...
    vault.timeleft = 6 * 30 * 24 * 60 * 60 * 1000; // 6 months
    vault.warned = false;
}

// withdraw after 6 months
public fun withdraw(_cap: &OwnerCap, vault: &mut Vault, clock: &Clock): bool {
    let current_time = clock.timestamp_ms();
    assert!(current_time - vault.last_time >= vault.timeleft, ELocked);
    if (!vault.warned) {
        vault.warned = true;
        vault.timeleft = 7 * 24 * 60 * 60 * 1000; // 7 days
        return false
    };
...
use smartwill::countdown::sixMonths;
#[test_only]
