...
export const downloadAndDecrypt = async (
  ...
  setDecryptedTexts: (texts: string[]) => void,    // ← 新增这一行
...
if (decryptedTexts.length > 0) {
  // 例如你可以用 setDecryptedText 來把文字顯示在畫面上
  setDecryptedFileUrls([]);            // (如果你之前用這個 state 來顯示 URL，可以清空它)
  setDecryptedTexts(decryptedTexts);   // ← 假設你已準備好一個 state：const [texts, setDecryptedTexts] = useState<string[]>([])
